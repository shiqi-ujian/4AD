# 腾讯文档收集表 → 问题收集箱 读取桥（4AD）

群友通过腾讯文档收集表提交问题；本桥定时把新提交读入 `问题收集/inbox/`，由 agent（主会话 dsh-schedule 定时唤醒）自动 debug、修复、发布，并在网站「更新日志」页公示。

## 一次性设置

1. **创建收集表**：打开 [docs.qq.com](https://docs.qq.com) → 新建「收集表」，字段（与 5z 表单一致，保证读取桥列映射可用）：
   - 昵称（选填）
   - 页面/功能（如 `hexmap 六角格地图` / `翻译：法术列表` / `2_工具/combatmap`）
   - 问题标题
   - 问题描述
   - 期望行为
   - 复现步骤（选填）
