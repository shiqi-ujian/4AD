...
> 🌐 **网页版在线入口**：GitHub Pages 已配置自动构建，推送后访问 **`https://shiqi-ujian.github.io/4AD/`** 即可打开地图工具首页（含 hexmap / combatmap 在线版）。

### hexmap — 六角格沙盒地图（通用跑团地图，**活跃工具**）
...
- **数据持久化**：保存/加载 JSON，导出 PNG 截图；分享链接（`#m=`）打开时自动加载地图
- **无需安装**：浏览器直接打开即可使用

### combatmap — 战斗地图生成器（通用跑团，**活跃工具**）

- **格子战斗图**：地形、墙壁/门、标签、区域、线段、图片 token、自定义地形
- **单位 Token 层**：玩家/敌人/NPC/盟友，名称、HP、状态标记、血条，可拖动缩放
- **实时在线房间**：PeerJS P2P，DM 创建房间 → 玩家打开链接加入，全员自由编辑实时同步
- **智能模板**：房间 / 走廊 / 洞窟 / 空地 / 区域一键生成
- **导出**：PNG、Excel（.xlsx / .xls）、枭熊 Owlbear Rodeo 场景、图文例
- **紧凑分享**：工具栏「🔗 分享」可复制完整地图链接/数据串，打开链接自动导入
- **在线版**：[`combatmap.dist.html`](2_工具/combatmap.dist.html)（由 `build.js` 打包）

### 其他地图工具（维护/冻结）

> ⚠️ **vanilla**：以下工具目前**不再主动开发**，仅保留为存量。当前主力工具是 `hexmap` 与 `combatmap`。若确有需要再动旧工具，先确认没有已在主力工具里实现的功能。
