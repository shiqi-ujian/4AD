# 
# 4AD 本地自动同步脚本（改编自 chm-web/scripts/autosync.ps1）
...
# 本脚本只做拉取+推送，不做自动提交。
# 提交需要手动执行（提交是在本地、不受网络限制）；这里只测一下是否有未提交改动，仅供日志参考。
    ...
    if ($haveRemote) {
        ...
        if ($r.Ok) {
            ...
            if ($merged -gt 0) {
                ...
                Send-Notice ($RepoName + ' 已拉取更新') ("从远端拉合并入 " + $merged + " 条提交")
            }
        } else {
...
if ($pendingCount -le 0) {
    ...
    for ($i = 1; $i -le $AttemptsPerRun; $i++) {
        ...
        if ($p.Ok) {
            Write-Log '推送成功'
            $ok = $true
            Send-Notice ($RepoName + ' 推送成功') ("已自动推送 " + $pendingCount + " 个提交到 GitHub（" + $branch + " 分支）")
            break
        }
